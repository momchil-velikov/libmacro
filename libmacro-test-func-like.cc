...
TEST_F(simple_func_macros_with_param, param_subst) {
  ...
  out = libmacro::macro_expand("C C(u, ,) C(, v, ) C( , ,w) C(u,v,) C(u, ,w) C( ,v ,w) C(u,v,w)", &macros, 0);
...
TEST_F(rec_func_macros2, param_subst) {
  std::string out;
  out = libmacro::macro_expand("A()", &macros, 0);
  ASSERT_EQ("D(F(u,D(F(u,F(F(u,v,w),E(F(u,v,w),E(w,u)),w).,w),E(w,u)).,w),F(w,u,w).)", out);
  out = libmacro::macro_expand("B(a)", &macros, 0);
  ASSERT_EQ("F(F(a,F(F(a,v,w),F(F(a,v,w),F(w,a,w).,w).,w),w),E(F(a,F(F(a,v,w),F(F(a,v,w),F(w,a,w).,w).,w),w),E(w,a)),w).", out);
  out = libmacro::macro_expand("C(a)", &macros, 0);
  ASSERT_EQ("F(F(a,F(F(a,v,w),E(F(a,v,w),E(w,a)),w).,w),F(F(a,F(F(a,v,w),E(F(a,v,w),E(w,a)),w).,w),F(w,a,w).,w).,w)", out);
  out = libmacro::macro_expand("D(a,b)", &macros, 0);
...
class c11_std_example_macros2 : public ::testing::Test {
 protected:
  c11_std_example_macros2() {
    ...
    macros.add_define(17, "debug(s,t) printf(\"x\" # s \"= %d, x\" # t \"= %s\", x ## s, x ## t)");
...
TEST_F(c11_std_example_macros2, string_and_paste) {
  ...
  out = libmacro::macro_expand("fputs(str(strncmp(\"abc\\0d\", \"abc\", '\\4') == 0) str(: @\\n), s);", &macros, 0);
  ASSERT_EQ("fputs(\"strncmp(\\\"abc\\\\0d\\\", \\\"abc\\\", '\\\\4') == 0\" \": @\\n\", s);", out);
